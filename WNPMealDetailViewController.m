...
@interface WNPMealDetailViewController () <UITableViewDataSource, UITableViewDelegate, UIScrollViewDelegate, SINCallClientDelegate>
...
@implementation WNPMealDetailViewController
...
- (IBAction)makePhoneCallButtonTouchUpInside:(id)sender {
    ...
    if ([self.client isStarted]) {
        ...
        [self performSegueWithIdentifier:@"callView_meal" sender:call];
    }
}
...
- (IBAction)sendTextButtonTouchUpInside:(id)sender {
    ...
    if ([self.client isStarted]) {
        [self performSegueWithIdentifier:@"messageView_meal" sender:nil];
    }
}
